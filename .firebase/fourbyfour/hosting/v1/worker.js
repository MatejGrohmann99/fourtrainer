...
self.onmessage = function (msg) {
    if (msg.data.puzzle) {main(msg.data)}
        ...
        let isEdgePiece = new Array(this.pcCount).fill(false);
                  [8, 9, 10, 11, 12, 13, 14, 15, 22, 23, 24, 25, 26, 27, 33, 34, 35, 36, 42, 43, 44, 45, 50, 51].forEach(i => {
                      isEdgePiece[i] = true;
                  });
/**
...
class Puzzle {
    ...
    constructor (cubeOri, clockwiseMoves, clockwiseMoveStr, solvedState=null, moveWeightsMap=new Map()) {
         this.edgePartners = new Map([
            ...
            // Edge positions (move to constructor too)
            this.edgePositions = [
                [8, 12], [9, 13], [10, 14], [11, 15],
                [27, 24], [34, 36], [25, 22], [51, 50]
            ];
    ...
    // Add this NEW method - compress state but ignore edges
    compressArrIgnoringEdges(list) {
        ...
        if (isEdgePiece[i]) {  // Array lookup instead of Set.has()
...
*readPrun(state, partialSolve=[], showPostAdj, maxDepth=this.pruneDepth) {
    for (let m=0; m<this.moves.length; m++) {
        if (partialSolve.length == 0 || this.validPairs[partialSolve[partialSolve.length-1]][m]) {
            ...
            // CHANGED: Use isGoalState instead of just checking edges
            if (nextDistance === 0 && this.isGoalState(nextState)) {
...
*solve(state, searchDepth, showPostAdj, startDepth=0) {
    // ADDED: Check if already at goal
...
getEndAdjust(nextState) {
    ...
    isEdgesPaired(state) {
        if ((state[35] & this.posMask) !== 35) return false;
        if ((state[33] & this.posMask) !== 33) return false;
        if ((state[26] & this.posMask) !== 26) return false;
        if ((state[23] & this.posMask) !== 23) return false;
        if ((state[42] & this.posMask) !== 42) return false;
        if ((state[44] & this.posMask) !== 44) return false;
        if ((state[43] & this.posMask) !== 43) return false;
        if ((state[45] & this.posMask) !== 45) return false;
    ...
    isGoalState(state) {
        return this.isEdgesPaired(state);
    }
