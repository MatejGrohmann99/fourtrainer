...
import 'package:fourtrainer/src/domain/algorithms/four_flip_algorithms.g.dart';
...
void main() {
  group(
    'fourFlipAlgorithms:',
        () {
      test(
        'Every generated algorithm does flips UF, UR, UB, UL edges and does not effect DF DR DB DL',
            () {
          final failedTests = <String>[];

          int lineCount = 2;
          for (final algorithm in fourFlipAlgorithms) {
            ...
            try {
              ...
              // test UL edge
              final cubeStateULEdgeTuple = (cubeState.state[0][1][0], cubeState.state[4][0][1]);
              final endStateULEdgeTuple = (endState.state[0][1][0], endState.state[4][0][1]);

              expect(cubeStateULEdgeTuple.$1, endStateULEdgeTuple.$2);
              expect(cubeStateULEdgeTuple.$2, endStateULEdgeTuple.$1);
...
file:         package:fourtrainer/src/domain/algorithms/uf_ur_ub_flip_algorithms.g.dart $lineCount:0
algorithm:    $algorithm
$e
''');
            }
          ...
          if (failedTests.isNotEmpty) {
            failedTests.insert(
              0,
              'ufurubFlipAlgorithms test failed ${failedTests.length} times out of ${fourFlipAlgorithms.length} tests',
            );
